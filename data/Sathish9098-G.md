...
## [G-2] Gas-Saving Benefits of Same Value Checks for Boolean State Variables 

Saves ``2400 GAS``

This ensures that the value of the ``bool`` state variable is never overwritten with the same value. This will saves ``800 GAS``. Add same value check before assigning values 
...
## [G-3] ``Increment`` or ``decrement`` by 1 can use ``++X`` instead of ``X+=1`` saves gas in state variables 
...
## [G-4] Don't assign default values to variables to save gas 
...
## [G-5] Splitting if() statements that use && saves gas
...
## [G-6] Avoid declaring new variable when only used once inside the function 
